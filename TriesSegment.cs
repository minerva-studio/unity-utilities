...
namespace Minerva.Module
{
    ...
    public struct TriesSegment<TValue> : ITries<TValue>
    {
        ...
        Tries<TValue>.Node ITries<TValue>.Root => root;
        ...
        public readonly bool Add(string s, TValue value) => Add(Split(s), value);

        public readonly bool Add<T>(T prefix, TValue value) where T : IList<string> => root?.Add(prefix, value) ?? false;
        ...
        public readonly bool ContainsKey(string s) => ContainsKey(Split(s));

        public readonly bool ContainsKey<T>(T prefix) where T : IList<string> => root?.ContainsKey(prefix) ?? false;
        ...
        public readonly bool TryGetValue(string s, out TValue value) => TryGetValue(Split(s), out value);

        public readonly bool TryGetValue<T>(T prefix, out TValue value) where T : IList<string> => root?.TryGetValue(prefix, out value) ?? ((value = default) == null && false);

        public readonly TValue Get(string s) => Get(Split(s));
        ...
        public readonly void Set(string s, TValue value) => Set(Split(s), value);

        public readonly void Set<T>(T prefix, TValue value) where T : IList<string> => root?.Set(prefix, value);

        bool ITries<TValue>.TryGetNode<T>(T prefix, out Minerva.Module.Tries<TValue>.Node currentNode) => TryGetNode(prefix, out currentNode);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private readonly bool TryGetNode<T>(T prefix, out Tries<TValue>.Node node) where T : IList<string> => root?.TryGetNode(prefix, out node) ?? ((node = default) == null && false);
        ...
        public readonly bool Remove(KeyValuePair<string, TValue> item) => Remove(new KeyValuePair<string[], TValue>(Split(item.Key), item.Value));

        public readonly bool Remove<T>(KeyValuePair<T, TValue> item) where T : IList<string> => root?.Remove(item) ?? false;

        public readonly bool Contains(KeyValuePair<string, TValue> item) => Contains(new KeyValuePair<string[], TValue>(Split(item.Key), item.Value));
        ...
        private readonly string[] Split(string s) => Tries<TValue>.Split(s, separator);
