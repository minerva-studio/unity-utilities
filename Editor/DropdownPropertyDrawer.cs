    ...
    public class DropdownAttributeDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            try
            {
                ...
                GetItemNameCallback GetItemName = dropdownAttribute.hasItemName ? ((baseMaster, obj) =>
                {
                    return ReflectionSystem.GetValue(baseMaster, obj, dropdownAttribute.itemNameProperty)?.ToString();
                }) : ((baseMaster, master) =>
                {
                    if (master is Object)
                    {//is Unity Object
